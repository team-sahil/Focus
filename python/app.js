...
function renderSelectionChips(){
  ...
  }).join('') + (!isTimerActive ? `<div class="sel-chip add-chip" data-action="add-subject" style="border:1px dashed var(--accent); color:var(--accent); background:transparent;">＋</div>` : '');
  ...
  if(subjObj && ((subjObj.sub && subjObj.sub.length > 0) || !isTimerActive)) {
    ...
    if(subjObj.sub && subjObj.sub.length > 0) {
      ...
      }).join('') + (!isTimerActive ? `<div class="sel-chip add-chip" data-action="add-sub" style="border:1px dashed var(--accent); color:var(--accent); background:transparent;">＋</div>` : '');
    } else {
      selectedSubMenu = '';
      area2.innerHTML = (!isTimerActive ? `<div class="sel-chip add-chip" data-action="add-sub" style="border:1px dashed var(--accent); color:var(--accent); background:transparent;">＋</div>` : '');
    }
  } else {
  ...
  }).join('') + (!isTimerActive ? `<div class="sel-chip add-chip" data-action="add-worktype" style="border:1px dashed var(--accent); color:var(--accent); background:transparent;">＋</div>` : '');
}
...
// 2. Sub-subject logic
const subSubjectManageArea = document.getElementById('subSubjectManageArea');
const newSubSubjectInput = document.getElementById('newSubSubject');
const addSubSubjectBtn = document.getElementById('addSubSubjectBtn');

// Show Sub-subject input area based on subject selection
const oldRenderSelectionChips = renderSelectionChips;
renderSelectionChips = function() {
  oldRenderSelectionChips();
  if (selectedSubject && !isTimerActive) {
    subSubjectManageArea.style.display = 'block';
  } else {
    subSubjectManageArea.style.display = 'none';
  }
};

addSubSubjectBtn.addEventListener('click', async () => {
  if(!selectedSubject) return;
  const val = sanitizeHTML(newSubSubjectInput.value.trim());
  if(!val) return;
  const subjObj = prefs.subjects.find(s => s.name === selectedSubject);
  if(subjObj) {
    if(!subjObj.sub) subjObj.sub = [];
    if(subjObj.sub.includes(val)) {
      showToast('এই সাব-সাবজেক্ট আগেই আছে!'); return;
    }
    subjObj.sub.push(val);
    selectedSubMenu = val;
    newSubSubjectInput.value = '';
    await savePrefs();
    renderSelectionChips();
    showToast(`"${val}" সফলভাবে যোগ করা হয়েছে!`);
  }
});
